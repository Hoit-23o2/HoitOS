...
​	在打开ramfs目录文件时，SylixOS首先会创建一个目录结构体（DIR），用于保存ls命令需要查看的目录文件信息（主要保存已打开文件的fd）并返回。但这个DIR除了fd并没有保存其他信息，真正记录目录文件下每一个文件的文件名称和状态的结构体是（struct dirent）。那么SylixOS怎么通过DIR去获取struct dirent的？关键函数就是我们在文件系统内定义的**__ramFsReadDir**。
