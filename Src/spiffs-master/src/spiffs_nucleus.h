...
/* 
    spiffs_page_object_ix_header: SpanIX = 0
    spiffs_page_object_ix_header: SpanIX != 0
 */
...
// cache struct
typedef struct {
  u8_t cpage_count;       /* 页面数 */
  u32_t last_access;      /* 上一次访问 */
  u32_t cpage_use_map;    /* CachePage位图 */
...
#if SPIFFS_CACHE
/**
 * @brief Cache读，命中读Cache，未命中释放Cache读入至Cache
 * 
 * @param fs 文件头
 * @param op 相关操作
 * //@param fh 文件号（无用）
 ...
 * @param len 长度
 * @param dst 目标
 * @return s32_t 成功返回SPIFFS_OK，否则返回其他
 */
