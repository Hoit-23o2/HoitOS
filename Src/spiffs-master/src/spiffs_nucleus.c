...
/**
 * @brief Find object lookup entry containing given **id** with visitor.
 ...
 * 
 *        封装了对object lookup的访问，每次访问可以返回一些值，特定时候退出循环，这就是nb之处了
 * 
...
s32_t spiffs_obj_lu_find_entry_visitor(
  ...
  while (res == SPIFFS_OK && entry_count > 0) {
    int obj_lookup_page = cur_entry / entries_per_page;  /* 当前所在页面 */
...
s32_t spiffs_erase_block(
  ...
  /**
   * 每次擦除一个块后，向Block的Erase Count处写入fs max_erase_count
   */
...
/**
 * @brief lookup page扫描回掉函数，检查obj_id，若为-1表明空，为0表明被删除，否则被分配
 * 
 * @param fs 文件系统
 * @param obj_id object id
 * @param bix 块号
 * @param ix_entry entry索引 
 * @param user_const_p 用户常量
 * @param user_var_p 用户变量
 * @return s32_t 
 */
...
static s32_t spiffs_obj_lu_find_free_obj_id_bitmap_v(spiffs *fs, spiffs_obj_id id, spiffs_block_ix bix, int ix_entry,
  ...
  if (id != SPIFFS_OBJ_ID_FREE && id != SPIFFS_OBJ_ID_DELETED) {
    ...
    if (conflicting_name && (id & SPIFFS_OBJ_ID_IX_FLAG)) {
      spiffs_page_ix pix = SPIFFS_OBJ_LOOKUP_ENTRY_TO_PIX(fs, bix, ix_entry); /* 找到该页面 */
      int res;
      spiffs_page_object_ix_header objix_hdr;
      res = _spiffs_rd(fs, SPIFFS_OP_T_OBJ_LU2 | SPIFFS_OP_C_READ,
          0, SPIFFS_PAGE_TO_PADDR(fs, pix), sizeof(spiffs_page_object_ix_header), (u8_t *)&objix_hdr);  /* 读取Index页面Header */
      SPIFFS_CHECK_RES(res);
      if (objix_hdr.p_hdr.span_ix == 0 &&         /* 只有IndexPAGE为第一页时有name */
          (objix_hdr.p_hdr.flags & (SPIFFS_PH_FLAG_DELET | SPIFFS_PH_FLAG_FINAL | SPIFFS_PH_FLAG_IXDELE)) ==
              (SPIFFS_PH_FLAG_DELET | SPIFFS_PH_FLAG_IXDELE)) {
        if (strcmp((const char*)user_const_p, (char*)objix_hdr.name) == 0) {
          return SPIFFS_ERR_CONFLICTING_NAME;     /* 发现重名文件咯，GG */
        }
      }
    }

    id &= ~SPIFFS_OBJ_ID_IX_FLAG;                 /* 去掉最高位 */
    u32_t bit_ix = (id-min_obj_id) & 7;           /* 0x111 */
    int byte_ix = (id-min_obj_id) >> 3;           /* /8 */
    if (byte_ix >= 0 && (u32_t)byte_ix < SPIFFS_CFG_LOG_PAGE_SZ(fs)) {
      fs->work[byte_ix] |= (1<<bit_ix);           /* 置id对应的位图为已占用？ */
    }
  }
  return SPIFFS_VIS_COUNTINUE;
}
...
s32_t spiffs_obj_lu_find_free_obj_id(spiffs *fs, spiffs_obj_id *obj_id, const u8_t *conflicting_name) {
  s32_t res = SPIFFS_OK;          
  u32_t max_objects = (fs->block_count * SPIFFS_OBJ_LOOKUP_MAX_ENTRIES(fs)) / 2;  /* 最大Objects数，因为一个Object至少由一个Index和一个Data构成 */
  ...
  while (res == SPIFFS_OK && free_obj_id == SPIFFS_OBJ_ID_FREE) {
    if (state.max_obj_id - state.min_obj_id <= (spiffs_obj_id)SPIFFS_CFG_LOG_PAGE_SZ(fs)*8) {
      ...
      memset(fs->work, 0, SPIFFS_CFG_LOG_PAGE_SZ(fs));      /* 清空work，用于记录位图数据 */
      ...
      for (i = 0; i < SPIFFS_CFG_LOG_PAGE_SZ(fs); i++) {
        ...
        for (j = 0; j < 8; j++) {
          if ((mask & (1<<j)) == 0) {
            *obj_id = (i<<3)+j+state.min_obj_id;        /* 找到了一个空闲块 */
            return SPIFFS_OK;
          }
        }
      }
