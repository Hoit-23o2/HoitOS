...
PHOIT_FULL_DNODE __hoit_truncate_full_dnode(PHOIT_VOLUME pfs, PHOIT_FULL_DNODE pFullDnode, UINT offset, UINT length) {
    ...
    PCHAR read_buf = (PCHAR)lib_malloc(pRawInfo->totlen);     /* 注意避免内存泄露 */
    if (!read_buf) {
        return LW_NULL;
    }
    ...
    PCHAR write_buf = (PCHAR)lib_malloc(sizeof(struct HOIT_RAW_INODE) + length);  /* 注意避免内存泄露 */
    if (!write_buf) {
        lib_free(read_buf);
        return LW_NULL;
    }
    ...
    PHOIT_RAW_INFO pNewRawInfo = (PHOIT_RAW_INFO)lib_malloc(sizeof(struct HOIT_RAW_INFO)); /* 注意避免内存泄露 */
    if (!pNewRawInfo) {
        lib_free(write_buf);
        lib_free(read_buf);
        return LW_NULL;
    ...
    PHOIT_FULL_DNODE pNewFullDnode = (PHOIT_FULL_DNODE)lib_malloc(sizeof(struct HOIT_FULL_DNODE));  /* 注意避免内存泄露 */
    if (!pNewFullDnode) {
        lib_free(write_buf);
        lib_free(read_buf);
        return LW_NULL;
...
PHOIT_FULL_DNODE __hoit_bulid_full_dnode(PHOIT_VOLUME pfs, PHOIT_RAW_INFO pRawInfo) {
    PCHAR read_buf = (PCHAR)lib_malloc(pRawInfo->totlen);        /* 注意内存泄露 */
    hoitReadFromCache(pfs->HOITFS_cacheHdr, pRawInfo->phys_addr, read_buf, pRawInfo->totlen);
    PHOIT_RAW_INODE pRawInode = (PHOIT_RAW_INODE)read_buf;
    crc32_check(pRawInode);
