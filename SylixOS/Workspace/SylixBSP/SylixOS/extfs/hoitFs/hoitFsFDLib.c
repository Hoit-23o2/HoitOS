...
BOOL __hoit_delete_full_dnode(PHOIT_VOLUME pfs, PHOIT_FULL_DNODE pFullDnode, INT flag) {
	if (flag == 1) {	/* 删完 */
		PCHAR read_buf = (PCHAR)hoit_malloc(pfs, pFullDnode->HOITFD_raw_info->totlen);
...
//        }
		...
		hoit_free(pfs, read_buf, pFullDnode->HOITFD_raw_info->totlen);
		hoit_free(pfs, pFullDnode, sizeof(HOIT_FULL_DNODE));
		return LW_TRUE;
	}
	else {				/* 只删dnode */
		hoit_free(pfs, pFullDnode, sizeof(HOIT_FULL_DNODE));
...
PHOIT_FULL_DNODE __hoit_truncate_full_dnode(PHOIT_VOLUME pfs, PHOIT_FULL_DNODE pFullDnode, UINT offset, UINT length) {
    ...
    PCHAR read_buf = (PCHAR)hoit_malloc(pfs, pRawInfo->totlen);     /* 注意避免内存泄露 */
    if (!read_buf) {
        return LW_NULL;
    }
    ...
    if (!write_buf) {
        hoit_free(pfs, read_buf, pRawInfo->totlen);
        return LW_NULL;
    }
    ...
    pNewRawInode->crc = 0;
    UINT32 tempCRC = hoit_crc32_le(write_buf, sizeof(struct HOIT_RAW_INODE) + length);
    pNewRawInode->crc = tempCRC;

    __hoit_write_flash(pfs, write_buf, sizeof(struct HOIT_RAW_INODE) + length, &phys_addr, 1);

    PHOIT_RAW_INFO pNewRawInfo = (PHOIT_RAW_INFO)hoit_malloc(pfs, sizeof(struct HOIT_RAW_INFO)); /* 注意避免内存泄露 */
    if (!pNewRawInfo) {
        hoit_free(pfs, write_buf, sizeof(struct HOIT_RAW_INODE) + length);
        hoit_free(pfs, read_buf, pRawInfo->totlen);
        return LW_NULL;
    }
    ...
    PHOIT_FULL_DNODE pNewFullDnode = (PHOIT_FULL_DNODE)hoit_malloc(pfs, sizeof(struct HOIT_FULL_DNODE));  /* 注意避免内存泄露 */
    if (!pNewFullDnode) {
        hoit_free(pfs, write_buf, sizeof(struct HOIT_RAW_INODE) + length);
        hoit_free(pfs, read_buf, pRawInfo->totlen);
        return LW_NULL;
...
PHOIT_FULL_DNODE __hoit_write_full_dnode(PHOIT_INODE_INFO pInodeInfo, UINT offset, UINT size, PCHAR pContent, UINT needLog) {
    ...
    PCHAR pBuf = (PCHAR)hoit_malloc(pfs, sizeof(HOIT_RAW_INODE) + size);
    ...
    pRawInode->crc = 0;
    pRawInode->crc = hoit_crc32_le(pBuf, totlen);
    __hoit_write_flash(pfs, pBuf, totlen, &phys_addr, needLog);

    if(phys_addr == 1203048){
    ...
    hoit_free(pfs, pBuf, sizeof(HOIT_RAW_INODE) + size);
